...
- [ ] Replace the 6 `value_type` string matches with `from_tag` / enum routing:
...
- [ ] `csv.rs:261` `cell_value_to_text` `value.value_type.as_str()` → `CellType::from_tag(...)`.
- [ ] `table.rs:87` `cell_text` `cv.value_type.as_str()` → `CellType::from_tag(...)`.

## 3. Migrate `formula/bridge.rs` and `worksheet.rs` sites

- [ ] `bridge.rs:66` `match cv.value_type.as_str()` → enum match.
- [ ] `bridge.rs:114-138` internal `cv.value_type = "X"` set sites → use `CellType` variant
      constructors if available, else leave as string assignment (document why).
- [ ] `bridge.rs:337` `if cv.value_type == "Formula"` → `matches!(..., Some(CellType::Formula))`.
- [ ] `worksheet.rs:195` and `worksheet.rs:1074` `value_type` comparisons → `from_tag` / enum match.
...
- [ ] `value()` hot path `cell.rs:277` `match cv.value_type.as_str()` →
...
- [ ] `cargo test --features formula-eval` passes.
- [ ] `napi build` (or `npm run build`) succeeds; call site types unchanged.
- [ ] Run the 154 JS test suite; confirm no `value_type` round-trip / `value()`
      drifts (formulas, rich text, hyperlinks, dates, errors all unaffected).
- [ ] Grep confirms no remaining `value_type == "<Lit>"` / `value_type.as_str()`
