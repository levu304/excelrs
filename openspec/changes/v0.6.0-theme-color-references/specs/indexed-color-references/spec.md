...
The style reader SHALL resolve `<color indexed="N"/>` references to ARGB via
the standard 56-entry system color palette (ECMA-376 §18.8.27), honoring a
custom `<indexedColors>` override from `xl/theme1.xml` when present.
Resolution applies at the same three color parse sites as theme colors and
stores the result in the existing `color: Option<String>` ARGB field.
