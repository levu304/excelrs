...
`<is><r>` and shared-string `<si><r>` runs) into a `CellValue` with
`value_type === "RichText"` and `rich_text` equal to the ordered
`Vec<RichTextRun>`, where each run carries its `text` and per-run `Font` whose
attributes are resolved per run-level `<rPr>`:
...
- **bold/italic/underline** from `<b/>`/`<i/>`/`<u/>` honoring `val`: `val`
  ...
  in bool field).
- **color** resolved from `<color>` to a single ARGB hex string (`FFRRGGBB`) on
  `Font.color`: `rgb` attribute used directly; `theme="N"` resolved via the
  workbook `xl/theme/theme1.xml` scheme (slot + optional `tint`); `indexed="N"`
  resolved via the workbook color palette; `auto` ⇒ `"FF000000"`.

Both inline `<is>` and shared-string `<si>` paths SHALL parse runs through a
single shared extraction core, so run behavior — including the no-`<rFont>` →
`null` invariant — is identical across both paths and guarded by one test.
...
#### Scenario: Inline and shared-string paths share one extraction core

- **WHEN** a no-`<rFont>` run appears in an inline string (`<is><r>`) and in a
  shared string (`<si><r>`)
- **THEN** the run font is produced by the same extraction core for both
  inputs, so `font.name === null` holds identically; a single regression test
  guards both paths
