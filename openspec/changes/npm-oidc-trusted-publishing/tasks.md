...
- [ ] 4.1 Push the workflow changes and cut a test `v*` tag (or run
  `workflow_dispatch`) — confirm all four packages publish via OIDC and the
  verify + smoke-test gate passes.
- [ ] 4.2 Once an OIDC publish is confirmed green, delete the `NPM_TOKEN`
  secret from the repository's GitHub settings.
- [ ] 4.3 Confirm the change is archived and `release-verification` spec reflects the OIDC requirement.
