...
Track merged cell ranges on worksheets and expose membership queries, so merge state survives a write/read round-trip and callers can test membership (e.g. `Worksheet.isMerged`) without duplicating range state.
