...
- **Cached formula results (authorable round-trip)** — a formula cell may carry a caller- or Excel-supplied cached scalar (`<f>..</f><v>..</v>`). `Cell.value = { formula, number | string | boolean | errorValue | dateSerial }` stores the cached scalar; the writer emits `<v>` with the matching `t` attribute; the reader surfaces `cell.value` as the cached scalar and `cell.formula` as the formula text. This is independent of the evaluation engine (above) — no computation is performed. `cachedValue` remains the recalc-only accessor behind `formula-eval`.
