...
├── index.js                # Auto-generated JS entry point
├── glue.ts                 # Thin JS glue for overloads + type narrowing
├── Cargo.toml
├── package.json
...
#   index.js                 — JS entry point (auto-generated by napi-rs)
#   index.d.ts               — TypeScript declarations (auto-generated by napi-rs)
#   glue.ts                  — Thin JS glue (overloads, optional type narrowing)
```
...
**JS glue file (`glue.ts` at project root):**

The glue file handles napi-rs impedance mismatches. It is ~25 lines and contains no business logic.

```typescript
// glue.ts — thin adapter between Rust napi exports and exceljs-compatible API
import { Worksheet as NativeWorksheet, Row as NativeRow } from './index';

// Method overloading: getCell(address: string) vs getCell(row: number, col: number)
NativeWorksheet.prototype.getCell = function(a: string | number, b?: number) {
...
NativeRow.prototype.getCell = function(col: number | string) {
  return typeof col === 'number'
    ? this.getCellByColNum(col)
    : this.getCellByColLetter(col);
};

// Optional: narrow the cell value setter type from `any` to a union
// This is cosmetic — the Rust setter already handles type dispatch via serde_json::Value.
```
...
| 12 | Thin JS glue for overloaded methods | napi-rs does not support method overloading. A ~20-line glue file dispatches between `getCellByAddress` and `getCellByRC` |
